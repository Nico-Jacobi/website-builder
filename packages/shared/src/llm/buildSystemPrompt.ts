...
export function buildSystemPrompt({ surface, mode, locked }: BuildSystemPromptArgs): string {
    ...
    const initialPrompt = [
        ...
        '6. Use `Container` for grouping only when a section clearly needs a distinct background or max-width. Max ONE level of nesting — do not put Containers inside Containers. **Always populate `children` with real block specs** — never omit `children`, never use placeholder text, never copy the default. See the Container example in the Component Enhancements section below.',
        ...
        '',
        '### Container: grouped section',
        'Container wraps one or more content blocks into a section with a shared background tone and optional max-width.',
        'The `children` array is **required** and must contain real block specs — never leave it empty or use placeholder text.',
        '`paddingY` controls vertical spacing: `"none"`, `"sm"`, `"md"` (default), `"lg"`.',
        '`maxWidth` (optional pixels) constrains the inner column width.',
        'Container itself gets the `tone` — child blocks must NOT have their own `tone`.',
        'Example:',
        '```json',
        '{',
        '  "type": "Container",',
        ...
        '          "body": "Three simple steps to get started."',
        '        }',
        '      },',
        '      {',
        '        "type": "FeatureGrid",',
        ...
        '          "columns": "3",',
        '          "features": [',
        '            { "icon": "1️⃣", "heading": "Sign up", "body": "Create a free account in seconds." },',
        '            { "icon": "2️⃣", "heading": "Configure", "body": "Set your preferences and goals." },',
        '            { "icon": "3️⃣", "heading": "Launch", "body": "Go live and track results in real time." }',
        '          ]',
        '        }',
        '      }',
        '    ]',
